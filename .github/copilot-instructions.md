...
- etcd_module Actor/CSP 并发迁移：`.github/skills/actor-csp-migration/SKILL.md`
...
- 本仓库是 C++ 到 Go 的翻译工程：Go 侧 `keepalive` 语义按 `registration` 理解（对应 C++ keepalive 模块的注册/刷新职责）。
- 讨论与新增命名时，优先使用 `registration`/`registrar`/`manager` 语义，避免将其误解为底层心跳循环。
- 真正 lease 句柄心跳由 etcd clientv3 keepalive handle 能力提供；`registration` 包更偏向注册状态编排与数据刷新。
