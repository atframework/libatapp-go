...
3. 读模型链路：`ProjectionActor 收事件 -> 合并 Topology + Registration -> 发布 ExportSnapshot`。
4. 重建链路：`LeaseExpired -> service 标记 stale -> 新 LeaseGranted -> 重放写入 -> 发布新快照版本`。
...
2. 它不仅处理 topology，也处理 registration 索引投影（ByPath/ByName/ByID）。
...
LeaseActor         EventBus              RegistrationActor       ProjectionActor
    ...
    │                   │                        │ PutWithLease(byname)│
     ...
     │                   │                        │ （批量，带 timeout ctx）
    │                   │ ◄──────────────────────│                     │
    │                   │ RegistrationIndexChanged(bypath/byname/byid) │
    │                   │ ─────────────────────────────────────────────►│
     │                   │                        │                     │
...
统一快照重建规则：
    - LeaseGranted：刷新 Registration 子视图 lease 代次，并接收注册索引增量
    - LeaseExpired：Registration 子视图标记为 stale，等待下一次 LeaseGranted 后整体恢复
    - Watch SnapshotLoaded：重建 Topology 子视图
    - 对外读取始终只读最后一次原子发布的完整快照
```
...
type ExportSnapshot struct {
    Topology struct {
        Ready      bool
        LastHeader *pb.EtcdResponseHeader
        NodesByPath map[string]*TopologyInfo
    }
    Registration struct {
...
说明：RegistrationActor 负责写入 etcd；ProjectionActor 负责统一投影与导出，避免查询路径分散在多个 actor。
...
type EventEnvelope struct {
    ...
    Sequence   uint64        // Source 内单调递增序号
...
1. 每个 Source 维护本地 `Sequence++`，只保证 Source 内有序。
2. `DedupeKey` 规则：
